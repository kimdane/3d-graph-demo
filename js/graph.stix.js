 var baseUrl="https://fireclover.imk.im/_db/demo2/demo2/";
  /* global SpriteText */
  /* global THREE */
  /* global dat */
  /* global fetch */
...
function makeTextSprite( message, parameters )
{
    ...
    function filterGraphNodes(graph, nodesTypes) {
      ...
      nodesTypes.dockerImages && types.push('dockerImage');
      ...
      for (var i=0;i<links.length;i++) {
          if (types.includes(links[i].source._id.split('/')[0]) && types.includes(links[i].target._id.split('/')[0]) && links[i].type != 'temp') {
    ...
    gui.add(startNode, 'StartNode')
      .onFinishChange(node => graph && 
          fetch(baseUrl+'stix-graph/'+encodeURIComponent(node)).then(res => res.json())
    ...
    const graph = ForceGraph3D()
      (elem) // bi-graph
        .jsonUrl(baseUrl+'stix-graph/'+encodeURIComponent(startNode.StartNode))
        .nodeLabel(d => d?.description ? d.description : d.name)
//        .nodeLabel(d => d?.type == 'vulnerability' ? d.description : d.name)
        ...
        .onNodeRightClick(node => {
          let { nodes, links } = graph.graphData();
          fetch(baseUrl+'stix-bigraph/'+encodeURIComponent(node._id)).then(res => res.json()).then(data => {
